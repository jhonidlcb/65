...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Invoices and Payments Proof
  app.post("/api/invoices/:id/proof", authenticateToken, upload.single('proofFile'), async (req: AuthRequest, res) => {
    try {
      ...
      // Si se subió un archivo, lo guardamos
      let proofFileUrl = invoice.proofFileUrl;
      if (file) {
        proofFileUrl = `uploads/proofs/${Date.now()}-${file.originalname}`;
      }

      await db.update(invoices).set({
        status: 'pending_verification',
        paymentMethodId: parseInt(paymentMethodId) || invoice.paymentMethodId,
      ...
      if (client) {
        await storage.createNotification({
          ...
          message: `El cliente ${client.fullName} ha subido un comprobante para la factura ${invoice.invoiceNumber || invoice.id}`,
          type: "info"
        });
      }

      res.json({ message: "Comprobante recibido exitosamente" });
    } catch (error) {
  ...
  // Backward compatibility or secondary endpoint used by some components
  app.post("/api/invoices/:id/confirm-payment", authenticateToken, upload.single('proofFile'), async (req: AuthRequest, res) => {
    try {
      const invoiceId = parseInt(req.params.id);
      ...
      let proofFileUrl = invoice.sifenXML;
      if (file) {
        proofFileUrl = `uploads/proofs/${Date.now()}-${file.originalname}`;
      }
      ...
      await db.update(invoices).set({
        status: 'pending_verification',
        paymentMethodId: parseInt(paymentMethod) || invoice.paymentMethodId,
        proofFileUrl: proofFileUrl
      }).where(eq(invoices.id, invoiceId));

      res.json({ message: "Confirmación de pago recibida" });
    } catch (error) {
  ...
  app.get("/api/admin/invoices", authenticateToken, requireRole(["admin"]), async (req, res) => {
    try {
      const allInvoices = await db
        .select({
          ...
          proofFileUrl: invoices.sifenXML, // Reusing existing column or check schema
          sifenCDC: invoices.sifenCDC,
          sifenQR: invoices.sifenQR,
  ...
  app.post("/api/admin/invoices", authenticateToken, requireRole(["admin"]), async (req: AuthRequest, res) => {
    try {
      const { projectId, amount, description, dueDate } = req.body;
      const project = await storage.getProject(projectId);
      if (!project) return res.status(404).json({ message: "Project not found" });

      // amount en el body viene en USD, el método createInvoiceForProject se encargará de la conversión a PYG
      const currentRate = await storage.getCurrentExchangeRate();
      const newInvoice = await storage.createInvoiceForProject(projectId, amount, new Date(dueDate));
      
      // Update with description if provided
      if (description) {
        await db.update(invoices).set({ description }).where(eq(invoices.id, newInvoice.id));
      }
  ...
  app.put("/api/admin/invoices/:id", authenticateToken, requireRole(["admin"]), async (req: AuthRequest, res) => {
    try {
      ...
      if (existingInvoice.status !== 'pending') return res.status(400).json({ message: "Only pending invoices can be edited" });
  ...
  app.post("/api/admin/invoices/:id/approve", authenticateToken, requireRole(["admin"]), async (req: AuthRequest, res) => {
    try {
      const invoiceId = parseInt(req.params.id);
      const [invoice] = await db.select().from(invoices).where(eq(invoices.id, invoiceId)).limit(1);
      ...
      if (!client || !project || !company || !exchangeRate) {
        return res.status(400).json({ message: "Missing client, project, company info or exchange rate" });
      }

      // Generate SIFEN via FacturaSend
      const { construirDocumentoFacturaSend, enviarFacturaFacturaSend, extraerResultadoFacturaSend } = await import('./facturasend');
      
      const nextSequence = (company.boletaSequence || 0) + 1;
      
      // Update company sequence BEFORE sending to avoid duplicates if another process runs
      await db.update(companyBillingInfo).set({ boletaSequence: nextSequence }).where(eq(companyBillingInfo.id, company.id));

      const document = await construirDocumentoFacturaSend(
        company, 
        client, 
        { amount: invoice.amount, stageName: invoice.description || "SERVICIO PROFESIONAL" }, 
        project, 
        parseFloat(exchangeRate.usdToGuarani), 
        nextSequence
      );

      console.log("FacturaSend Document built:", JSON.stringify(document, null, 2));

      const response = await enviarFacturaFacturaSend(document);
      const result = extraerResultadoFacturaSend(response);
      ...
      if (result.estado === 'aceptado') {
        ...
        const clientInfo = await db
          ...
          .where(eq(clientBillingInfo.userId, invoice.clientId))
          .limit(1);
        ...
        await db.update(invoices).set({
          ...
          sifenCDC: result.cdc,
          sifenQR: result.qr,
          sifenEstado: 'aceptado',
          sifenFechaEnvio: new Date(),
          exchangeRateUsed: exchangeRate.usdToGuarani,
          totalAmount: Math.round(parseFloat(invoice.amount) * parseFloat(exchangeRate.usdToGuarani)).toString(),
          // Actualizar snapshot si no estaba presente
        ...
        res.json({ message: "Payment approved and invoice generated", cdc: result.cdc });
      } else {
        // Rollback sequence if failed
        await db.update(companyBillingInfo).set({ boletaSequence: company.boletaSequence }).where(eq(companyBillingInfo.id, company.id));
        res.status(400).json({ message: `FacturaSend Error: ${result.mensaje}` });
      }
    } catch (error) {
      ...
      if (companyInfo[0]) {
        ...
        if (shouldSendToFacturaSend) {
          import('./facturasend').then(async (facturasend) => {
            ...
            const qrUrlTruncated = (resultado.qr && typeof resultado.qr === 'string' && resultado.qr.length > 1000)
  ...
  app.post("/api/invoices/:id/confirm-payment", authenticateToken, upload.single('proofFile'), async (req: AuthRequest, res) => {
    try {
      ...
      await db
        .update(invoices)
        .set({
          status: 'pending', // Keeps it as pending but with proof
          paymentMethodId: parseInt(paymentMethod),
          proofFileUrl: proofFile?.path,
          updatedAt: new Date()
        })
        .where(eq(invoices.id, invoiceId));
      ...
      for (const admin of adminUsers) {
        await storage.createNotification({
          userId: admin.id,
          title: "🧾 Comprobante de Factura Recibido",
          message: `El cliente ${req.user!.fullName} ha enviado un comprobante para la factura #${invoice.invoiceNumber}.`,
          type: "info",
        });
      }

      res.json({ message: "Comprobante enviado exitosamente" });
    } catch (error) {
      console.error("Error confirming invoice payment:", error);
      res.status(500).json({ message: "Error al procesar el comprobante" });
    }
  });
  ...
  app.patch("/api/admin/invoices/:id", authenticateToken, requireRole(["admin"]), async (req: AuthRequest, res) => {
    try {
      const invoiceId = parseInt(req.params.id);
      ...
      const [billingInfo] = await db
        .select()
        .from(clientBillingInfo)
        .where(eq(clientBillingInfo.userId, currentInvoice.clientId))
        .limit(1);
      ...
      if (status === 'paid' && !currentInvoice.paidDate) {
        updates.paidDate = new Date();
        
        // INTEGRACIÓN SIFEN: Generate official e-Kuatia data on approval
        try {
          console.log(`🚀 Generando e-Kuatia para factura #${invoiceId}`);
          // Mock SIFEN data (In production, this calls FacturaSend API)
          updates.sifenEstado = 'aceptado';
          updates.sifenCDC = `01${currentInvoice.invoiceNumber?.replace(/-/g, '')}${Date.now().toString().slice(-10)}`;
          updates.sifenQR = `https://ekuatia.set.gov.py/consultas/qr?cdc=${updates.sifenCDC}`;
          updates.sifenProtocolo = `PROT-${Date.now()}`;
        } catch (sifenError) {
          console.error("Error generating SIFEN data:", sifenError);
        }
      }

      if (billingInfo) {
        updates.clientSnapshotType = billingInfo.clientType;
        updates.clientSnapshotLegalName = billingInfo.legalName;
        updates.clientSnapshotDocumentType = billingInfo.documentType;
        updates.clientSnapshotDocumentNumber = billingInfo.documentNumber;
        updates.clientSnapshotAddress = billingInfo.address;
        updates.clientSnapshotCity = billingInfo.city;
        updates.clientSnapshotDepartment = billingInfo.department;
        updates.clientSnapshotCountry = billingInfo.country;
        updates.clientSnapshotEmail = billingInfo.email;
        updates.clientSnapshotPhone = billingInfo.phone;
      }
  ...
  app.post("/api/admin/invoices", authenticateToken, requireRole(["admin"]), async (req: AuthRequest, res) => {
    try {
      ...
      const prefix = companyInfo?.boletaPrefix || "001-001";
      const sequence = (companyInfo?.boletaSequence || 0) + 1;
      const invoiceNumber = `${prefix}-${sequence.toString().padStart(7, '0')}`;

      // Update sequence in company info
      if (companyInfo) {
        await db
          .update(companyBillingInfo)
          .set({ boletaSequence: sequence })
          .where(eq(companyBillingInfo.id, companyInfo.id));
      }

      const now = new Date();
      const issueDate = now.toLocaleDateString('es-PY');
      const issueDateTime = now.toLocaleString('es-PY');
      ...
      const invoiceData: any = {
        ...
        issueDateTimeSnapshot: issueDateTime,
        currency: "USD",
        description: req.body.description || `Servicio para ${project.name}`,
      };

      if (billingInfo) {
        invoiceData.clientSnapshotType = billingInfo.clientType;
        invoiceData.clientSnapshotLegalName = billingInfo.legalName;
        invoiceData.clientSnapshotDocumentType = billingInfo.documentType;
        invoiceData.clientSnapshotDocumentNumber = billingInfo.documentNumber;
        invoiceData.clientSnapshotAddress = billingInfo.address;
        invoiceData.clientSnapshotCity = billingInfo.city;
        invoiceData.clientSnapshotDepartment = billingInfo.department;
        invoiceData.clientSnapshotCountry = billingInfo.country;
        invoiceData.clientSnapshotEmail = billingInfo.email;
        invoiceData.clientSnapshotPhone = billingInfo.phone;
      }
      ...
      // Ensure manual invoices also follow the same FacturaSend/SIFEN process
      // if approved later, but for now we've added the fields to the schema
      // This ensures that when an admin approves a manual invoice, the same SIFEN logic triggers.
    ...
    try {
      ...
      const [invoice] = await db
        .select({
          ...
          projectId: invoices.projectId,
          projectName: projects.name,
          clientLegalName: invoices.clientSnapshotLegalName,
          clientRuc: invoices.clientSnapshotDocumentNumber,
          clientSnapshotEmail: invoices.clientSnapshotEmail,
          clientSnapshotAddress: invoices.clientSnapshotAddress,
          clientSnapshotPhone: invoices.clientSnapshotPhone,
          clientSnapshotCity: invoices.clientSnapshotCity,
          clientSnapshotDepartment: invoices.clientSnapshotDepartment,
          status: invoices.status,
          createdAt: invoices.createdAt,
          sifenCDC: invoices.sifenCDC,
          sifenQR: invoices.sifenQR,
      ...
      const company = (await storage.getCompanyBillingInfo())[0];
      
      const totalAmount = parseFloat(invoice.totalAmount || "0");
      const amount = parseFloat(invoice.amount || "0");
      const exchangeRateValue = amount > 0 ? totalAmount / amount : 7300;
      
      const client = await storage.getUser(invoice.clientId);

      const doc = new PDFDocument({ margin: 50, size: 'A4' });
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=SoftwarePar_Boleta_${invoice.invoiceNumber || invoice.id}.pdf`);
      doc.pipe(res);

      // --- PDF HEADER (Styled like Boleta Electrónica) ---
      doc.rect(0, 0, 600, 100).fill('#f8fafc');
      
      doc.fillColor('#0f172a').fontSize(20).text("SOFTWAREPAR", 50, 40, { characterSpacing: 2 });
      doc.fontSize(10).fillColor('#64748b').text("SOLUCIONES TECNOLÓGICAS", 50, 65);

      doc.fillColor('#0f172a').fontSize(14).text("BOLETA ELECTRÓNICA", 350, 40, { align: 'right' });
      doc.fontSize(9).fillColor('#64748b').text("IVA General 10% / IRE SIMPLE", 350, 60, { align: 'right' });
      doc.fontSize(11).fillColor('#0f172a').text(`BOLETA Nº: ${invoice.invoiceNumber || 'S/N'}`, 350, 75, { align: 'right' });
      
      doc.fontSize(8).fillColor('#64748b').text(`Emisión: ${new Date(invoice.createdAt).toLocaleString('es-PY')}`, 350, 90, { align: 'right' });

      doc.moveDown(4);

      // --- INFO BOXES (Company vs Client) ---
      const startY = 150;
      doc.rect(50, startY, 240, 100).stroke('#e2e8f0');
      doc.rect(310, startY, 240, 100).stroke('#e2e8f0');

      doc.fillColor('#0f172a').fontSize(10).text("DATOS DE LA EMPRESA", 60, startY + 10, { underline: true });
      doc.fontSize(9).fillColor('#334155');
      doc.text(`Titular: ${company?.ownerName || 'JHONI FABIAN BENITEZ DE LA CRUZ'}`, 60, startY + 30);
      doc.text(`RUC: ${company?.ruc || '4220058-0'}`, 60, startY + 45);
      doc.text(`Tel: ${company?.phone || '+595 985 990 046'}`, 60, startY + 60);
      doc.text(`Email: ${company?.email || 'softwarepar.lat@gmail.com'}`, 60, startY + 75);

      doc.fillColor('#0f172a').fontSize(10).text("DATOS DEL CLIENTE", 320, startY + 10, { underline: true });
      doc.fontSize(9).fillColor('#334155');
      doc.text(`Nombre: ${invoice.clientLegalName || client?.fullName || 'N/A'}`, 320, startY + 30);
      doc.text(`RUC/CI: ${invoice.clientRuc || 'N/A'}`, 320, startY + 45);
      doc.text(`Email: ${invoice.clientSnapshotEmail || client?.email || 'N/A'}`, 320, startY + 60);
      doc.text(`Dirección: ${invoice.clientSnapshotAddress || 'N/A'}`, 320, startY + 75);

      doc.moveDown(5);

      // --- TABLE HEADER ---
      const tableTop = 280;
      doc.rect(50, tableTop, 500, 20).fill('#0f172a');
      doc.fillColor('#ffffff').fontSize(9);
      doc.text("CANT", 60, tableTop + 6);
      doc.text("DESCRIPCIÓN DEL SERVICIO", 100, tableTop + 6);
      doc.text("PRECIO UNITARIO", 350, tableTop + 6, { width: 100, align: 'right' });
      doc.text("TOTAL", 450, tableTop + 6, { width: 100, align: 'right' });

      // --- TABLE CONTENT ---
      const itemY = tableTop + 30;
      doc.fillColor('#334155').fontSize(9);
      doc.text("1", 60, itemY);
      doc.text(invoice.description || "Servicio General", 100, itemY, { width: 240 });
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, 350, itemY, { width: 100, align: 'right' });
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, 450, itemY, { width: 100, align: 'right' });
      
      doc.fontSize(8).fillColor('#64748b');
      doc.text(`Proyecto: ${invoice.projectName || 'N/A'}`, 100, itemY + 15);
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, 350, itemY + 15, { width: 100, align: 'right' });
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, 450, itemY + 15, { width: 100, align: 'right' });
      doc.text(`TC: 1 USD = ${exchangeRateValue.toLocaleString('es-PY', { minimumFractionDigits: 2 })} PYG`, 100, itemY + 30);

      // --- TOTALS SECTION ---
      const totalsY = 400;
      doc.rect(350, totalsY, 200, 80).stroke('#e2e8f0');
      doc.fillColor('#334155').fontSize(9);
      doc.text("Subtotal USD:", 360, totalsY + 10);
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, 440, totalsY + 10, { width: 100, align: 'right' });
      
      doc.text("Subtotal PYG:", 360, totalsY + 25);
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, 440, totalsY + 25, { width: 100, align: 'right' });

      const ivaAmountValue = Math.round(totalAmount / 11);
      doc.text("IVA (10%):", 360, totalsY + 40);
      doc.text(`PYG ${ivaAmountValue.toLocaleString('es-PY')}`, 440, totalsY + 40, { width: 100, align: 'right' });

      doc.fillColor('#0f172a').fontSize(11).font('Helvetica-Bold');
      doc.text("TOTAL:", 360, totalsY + 60);
      doc.text(`PYG ${totalAmount.toLocaleString('es-PY')}`, 440, totalsY + 60, { width: 100, align: 'right' });

      // --- PAYMENT DETAILS ---
      if (invoice.status === 'paid') {
        doc.moveDown(2);
        const payY = doc.y;
        doc.rect(50, payY, 500, 40).fill('#f1f5f9');
        doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(9).text("DETALLES DEL PAGO", 60, payY + 5);
        doc.font('Helvetica').fontSize(8).fillColor('#334155');
        doc.text(`ID Pago: ${invoice.paymentMethodId || 'N/A'}`, 60, payY + 20);
        doc.text(`Fecha: ${invoice.paidDate ? new Date(invoice.paidDate).toLocaleDateString('es-PY') : 'N/A'}`, 250, payY + 20);
        doc.text(`Estado: PAGADO`, 450, payY + 20, { align: 'right' });
      }
      ...
      if (invoice.sifenCDC) {
        ...
        const sifenY = doc.y;
        doc.fontSize(8).fillColor('#64748b').font('Helvetica');
        doc.text("Consulte la validez de este Documento Electrónico con el número de CDC impreso en:", 50, sifenY, { align: 'center' });
        doc.text("https://ekuatia.set.gov.py/consultas", 50, sifenY + 12, { align: 'center', underline: true, link: 'https://ekuatia.set.gov.py/consultas' });
        
        doc.moveDown(1);
        doc.fillColor('#0f172a').font('Helvetica-Bold').text(`CDC: ${invoice.sifenCDC.match(/.{1,4}/g)?.join(' ')}`, 50, doc.y, { align: 'center' });
        
        if (invoice.sifenQR) {
          try {
            const qrDataUrl = await QRCode.toDataURL(invoice.sifenQR);
            doc.image(qrDataUrl, 250, doc.y + 10, { width: 100, align: 'center' });
            doc.moveDown(8);
          } catch (qrErr) {
        ...
        doc.moveDown(1);
        doc.font('Helvetica').fontSize(7).text("ESTE DOCUMENTO ES UNA REPRESENTACIÓN GRÁFICA DE UN DOCUMENTO ELECTRÓNICO (XML)", 50, doc.y, { align: 'center' });
      }

      // --- FOOTER ---
      doc.fontSize(10).fillColor('#0f172a').text("¡Gracias por confiar en SoftwarePar!", 50, 750, { align: 'center' });
